...
APPEND HELLGEN
  ...
  IF ~Global("C7Geas", "GLOBAL", 2) PartyHasItem("A7DJLMP") Global("A7PrepareAfaaq", "LOCALS", 0)~ AfaaqUpgrade.2
    SAY ~It is done, my <PRO_LADYLORD>. Now that my freedom is guaranteed, I may help you too.~
    = ~You appear to have one of my own kind in your midst. If you lend me your sword of power for a moment, I can free your djinn from his shackles. Do not worry, the sword will not be damaged in the process.~
    IF ~~ DO ~GiveItem("MISCB7",LastTalkedToBy) AddexperienceParty(20000) SetGlobal("Player1Greed","GLOBAL",3)~ + AfaaqUpgrade.3
  END

  IF ~~ AfaaqUpgrade.3
    SAY ~Let me prepare the spell...~
  ...
  IF ~~ THEN AfaaqUpgrade.6
    SAY ~I thank you again for my release. Good luck in whatever trials you are to face, my <PRO_LADYLORD>. And may your future shine brightly, Afaaq.~
  ...
  IF ~~ THEN AfaaqUpgrade.7
    SAY ~Good luck in whatever trials you are to face, my <PRO_LADYLORD>. And may your future shine brightly, Afaaq.~
    IF ~~ DO ~ActionOverride(Player1, CreateItem("A7DJLMPA", 0, 0, 0)) CreateVisualEffect("spplanar",[362.918]) Wait(2) DestroySelf()~ EXIT
  END
END
